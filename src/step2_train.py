...
class ELDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, umls, max_length=512, k=10):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.k):
            if candidate_CUI[i] in self.umls and self.umls[candidate_CUI[i]] is not None:
                sentence1 += f" [ST{i}] {self.umls[candidate_CUI[i]]}"
            else:
                sentence1 += f" [ST{i}] {candidates[i]}"
        inputs = self.tokenizer(sentence0, sentence1, max_length=self.max_length, truncation=True, padding=True)
        inputs["label"] = labels
        return inputs
...
class ELCollator:
    ...
    def __call__(self, inputs):
        ...
        for i in range(batch_size):
            input_ids.append(inputs[i]["input_ids"])
            attention_mask.append(inputs[i]["attention_mask"])
            token_type_ids.append(inputs[i]["token_type_ids"])
            labels.append(inputs[i]["label"])
...
def get_umls():
    ...
    umls_map = {}
    for i in umls:
        umls_map[i["ENTITY"]] = i["DEFINITION"]
    return umls_map
...
def main(args):
    ...
    train_dataset = ELDataset(train_data, tokenizer, umls, args.max_length)
    dev_dataset = ELDataset(dev_data, tokenizer, umls, args.max_length)
    train_dataloader = DataLoader(train_dataset, batch_size=args.train_batch_size, collate_fn=ELCollator(tokenizer))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--retrieval_topk", type=int, default=10, help="Top k retrieval results")
